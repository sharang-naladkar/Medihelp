...
def upgrade():
    ...
    op.bulk_insert(sa.table("drones",sa.column("id",sa.String),sa.column("callsign",sa.String),sa.column("last_lat",sa.Float),sa.column("last_lng",sa.Float),sa.column("status",sa.String),sa.column("battery_pct",sa.Float)),[{"id":os.environ["DRONE_ID"],"callsign":"MEDIDRONE-01","last_lat":float(os.environ["DRONE_HOME_LAT"]),"last_lng":float(os.environ["DRONE_HOME_LNG"]),"status":"AVAILABLE","battery_pct":None}])
def downgrade():
